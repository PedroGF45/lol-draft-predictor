...
class FeatureEngineer:
    ...
    def perform_feature_engineering(
        ...
        self.logger.info(f"Feature engineering completed. Final train shape: {data_train.shape}, test shape: {data_test.shape}")
    ...
    def generate_new_features(self, dataframe: pd.DataFrame) -> pd.DataFrame:
        ...
        for team in teams:
            for role in roles:
                ...
                if tier_col in df.columns and rank_col in df.columns:
                    df[score_col] = (
                        df[tier_col].map(tier_scores).fillna(0) + 
                        df[rank_col].map(rank_scores).fillna(0)
                    )
        
        # ===== Team Rank Score Statistics =====
        # Calculate team-level rank statistics
        for team in teams:
            rank_score_cols = [f'team{team}_{role}_rank_score' for role in roles 
                              if f'team{team}_{role}_rank_score' in df.columns]
        ...
        for team in teams:
            if all(f'team{team}_avg_{stat}' in df.columns for stat in ['kills', 'deaths', 'assists']):
                df[f'team{team}_kda'] = (
                    (df[f'team{team}_avg_kills'] + df[f'team{team}_avg_assists']) / 
                    df[f'team{team}_avg_deaths'].replace(0, 1)
                )
        ...
        for team in teams:
            pick_cols = [f'team{team}_pick_{role}' for role in roles 
                        if f'team{team}_pick_{role}' in df.columns]
            if pick_cols:
        ...
        metric_pairs = [
            "avg_wins", "avg_losses", "avg_kills", "avg_deaths", "avg_assists",
            "avg_cs", "avg_gold_earned", "avg_damage_dealt", "avg_damage_taken",
            "avg_vision_score", "avg_healing_done",
            "cs_per_minute", "gold_per_minute", "damage_per_minute",
            "vision_score_per_minute", "healing_per_minute",
            "win_rate", "ranked_league_points", "summoner_level",
            "champion_mastery_level", "champion_total_mastery_score",
        ]
    ...
    def one_hot_encode(self, data_train: pd.DataFrame, data_test: pd.DataFrame, 
                       data_handler: DataHandler) -> tuple[pd.DataFrame, pd.DataFrame]:
        """
        ...
        self.logger.info(f"One-hot encoding completed. New shape: train={train_encoded.shape}, test={test_encoded.shape}")
        return train_encoded, test_encoded

    def transform_numerical_features(self, data_train: pd.DataFrame, data_test: pd.DataFrame,
                                    data_handler: DataHandler) -> tuple[pd.DataFrame, pd.DataFrame]:
        """
        ...
        self.quantile_transformer = QuantileTransformer(
            n_quantiles=min(1000, len(data_train)),
        ...
        train_transformed[existing_numerical] = self.quantile_transformer.fit_transform(
            data_train[existing_numerical]
        )
        test_transformed[existing_numerical] = self.quantile_transformer.transform(
            data_test[existing_numerical]
        )
        
        self.logger.info("QuantileTransformer applied successfully")
        return train_transformed, test_transformed

    def normalize_features(self, data_train: pd.DataFrame, data_test: pd.DataFrame,
                          data_handler: DataHandler) -> tuple[pd.DataFrame, pd.DataFrame]:
        """
        ...
        train_normalized[existing_numerical] = self.standard_scaler.fit_transform(
            data_train[existing_numerical]
        )
        test_normalized[existing_numerical] = self.standard_scaler.transform(
            data_test[existing_numerical]
        )
